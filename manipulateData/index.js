const sortData = (data) => {
  if (data !== null && data !== undefined && data.length > 0) {
    ...
    const sortedDataArray = sortTempArray.map((e) => data[e.index]);
...
const splitArray = async (data) => {
  if (data !== undefined && data.length > 0) {
    ...
    // });
    const stopsRaw = await data.map((e) => ({
      // to be modified to dataModified.map
      name: e.stop.name,
      order: e.order
    }));
    const intermediateArray = await stopsRaw.reduce((acc, item) => {
      const arr = acc.slice();
    ...
    const resultArray = await data.reduce((acc, curr) => {
      // to be modified to dataModified.reduce
      const arr = [...acc];
